...
st.markdown("""
    ...
    - **Giả định trung thức (Faithfulness Assumption)**: Mọi độc lập có điều kiện trong phân phối dữ liệu đều phản ánh cấu trúc đồ thị nhân quả, tức là không có sự phụ thuộc ngẫu nhiên nào xảy ra.
""")
...
with col1:
    st.latex("V_{\mathcal{K}} = \{x^K_1, x^K_2, \ldots, x^K_n\}")
with col2:
    st.latex("E_{\mathcal{K}} = \{(x^K_i, x^K_j) | x^K_i, x^K_j \in V_{\mathcal{K}}\}")

st.markdown(r"""và mạng nhân quả $$\mathcal{G^*}$$ với bộ tham số và tập cạnh""")
col1, col2 = st.columns(2)
with col1:
    st.latex("X = \{x^G_1, x^G_2, \ldots, x^G_n\} ")
with col2:
    st.latex("E_{\mathcal{G^*}} = \{(x^G_i, x^G_j) | x^G_i, x^G_j \in X\}")
...
st.markdown(r"""
    [**Lọc mẫu ổn định ngẫu nhiên (Stability Selection)**](https://arxiv.org/pdf/0809.2932) là một kỹ thuật lọc phổ biến trong thống kê và học máy, giúp cải thiện độ tin cậy của việc lẫy mẫu bằng cách lặp lại quá trình lựa chọn trên các tập con khác nhau của dữ liệu.
